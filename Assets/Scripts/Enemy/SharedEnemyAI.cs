...
public class SharedEnemyAI : MonoBehaviour
{
    ...
    public void RegisterAgent(EnemyAgent agent, GameObject Agent)
    {
    ...
    public void SearchInRangeAgent(EnemyAgent notifierAgent, int range, bool pDevice = false)
    {
        ...
        foreach (var otherAgent in _agents)
        {
            if (otherAgent != notifierAgent)
            {
                ...
                {
                    NotifyAgentOfPos(notifierAgent.LastSeenTargetPosition, otherAgent, pDevice);
    ...
    public void SearchInRangeVec3(Vector3 location, int range, bool pDevice = false)
    {
        ...
        foreach (var otherAgent in _agents)
        {
            ...
            {
                NotifyAgentOfPos(location, otherAgent, pDevice);
                otherAgent.SetState(typeof(ChaseState));
                searchingAgents++;
            }
        }
